sap.ui.define([
			...
			return BaseController.extend("zclaimProcessing.controller.PartsMainSection", {
					...
					if (!oValid01 && !oValid02) {
						...
						if (this.claimType != "ZPPD") {
							...
							if (this.addPartFlag == true || this.updatePartFlag == true) {
								...
								if (this.getModel("LocalDataModel").getProperty("/BaseUnit") == "/PC" || this.getView().getModel("LocalDataModel").getProperty(
										"/BaseUnit") == '***') // 7-JuLY-23  Shriram  INC0235295 Code start
								{
				...
				_fnValidatorParts: function () {

					var oClmType = this.getView().getModel("HeadSetData").getProperty("/WarrantyClaimType");
					var that = this;
					var aInputs;
					var zpdczptsArr = [
						this.byId("idOutboundDNum"),
						this.byId("idShipmentRDate"),
						this.byId("idDealerContact"),
						this.byId("idTCIWayBill"),
						this.byId("idCarrierName")
					];
						...
						// 	}
							...
							if (this._fnValidatorParts()) {
								...
								this.getView().getModel("HeadSetData").getProperty("/ShipmentReceivedDate", this.getView().getModel("HeadSetData").getProperty(
									"/ShipmentReceivedDate").getTime() - (10.5 * 60 * 60));
								var that = this;
								oFilteredDealerData = this.getView().byId("idCarrierName").getModel("BpDealerModel").getData().BpDealerList.filter(function (
									val) {
										...
										new Button({
											...
											press: $.proxy(function () {
												...
												if (this._fnValidatorParts()) {
													...
													this.getView().byId("idMainClaimMessage").setProperty("visible", true);
													this.getView().getModel("DateModel").setProperty("/SubmitPWBusyIndicator", false);
												} else if ((enabledIntent > -1 && this.claimType === "ZPDC" && oAttachmentCheck == -1 && oPartDiscDes == -1) ||
